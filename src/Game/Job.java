...
public class Job implements Defs, Stressor.Causable {
	...
	private final static Act GatherMillet = Act.newReapC("Gathering Edibles",	N(),						I(millet),		null,			Shire.FERTILITY,	FARMER,		1000); //vegetation
	private final static Act HuntSloth = Act.newReapC("Hunting Sloth",			N(),						I(meat),		P_.MARKSMANSHIP,Shire.WILDERNESS,	HERDER,		1000);
	private final static Act Settle = Act.newReapR("Settling Land",				N(),						I(land),		null,			Shire.WILDERNESS,	LUMBERJACK, 100);
	...
	private final static Act Butcher = Act.newCraft("Butchering Animal",		OR(I(bovad), I(donkey),
																				I(lobodonkey)),				M(3, meat),		null,								LOBOTOMIST,	1000);
	private final static Act DungCollecting = Act.newCraft("Collecting Dung",	I(rentanimal),				I(poop),		null,								-1,			-1);
	private final static Act Lumberjacking = Act.newReapR("Felling Trees",		N(),						I(timber),		P_.STRENGTH,	Shire.TREES,		CARPENTER,	500);
	private final static Act Quarry = Act.newReapR("Quarrying Stone",			N(),						I(stone),		P_.STRENGTH,	Shire.STONES,		BUILDER,	500);
	...
	private final static Act MakeBow = Act.newCraft("Crafting Crossbow",		I(timber),					I(bow),			P_.CARPENTRY,						-1,			-1);
	private final static Act ForgeWeapon = Act.newCraft("Smithing Weapon",		AND(iron, timber),			OR(sword, mace),P_.SMITHING,						-1,			-1);
	private final static Act ForgeArmor = Act.newCraft("Smithing Armor",		AND(M(3, iron), I(timber)),	I(armor),		P_.SMITHING,						-1,			-1);
	private final static Act ForgeGun = Act.newCraft("Smithing Arquebus",		AND(I(iron), M(2, timber)),	I(gun),			P_.SMITHING,						-1,			-1);
	private final static Act ConstructS = Act.newCraft("Building Construction",	I(stone),					I(sconstr),		P_.MASONRY,							-1,			-1);
	private final static Act ConstructW = Act.newCraft("Building Construction",	I(timber),					I(wconstr),		P_.CARPENTRY,						-1,			-1);
	private final static Act Build = Act.newCraft("Constructing Building",		OR(wconstr, sconstr),		I(constr),		P_.STRENGTH,						-1,			-1);
	
	private final static Act TradeA = Act.newTrade("Trading Animals",			OR(bovad, donkey, lobodonkey, poop),											-1,			-1);
	private final static Act TradeC = Act.newTrade("Trading Commodities",		OR(timber, stone, iron, silver, constr),										-1,			-1);
	private final static Act TradeW = Act.newTrade("Trading Arms",				OR(sword, mace, bow, gun, armor),												-1,			-1);
	private final static Act Lobotomize = Act.newCraft("Lobotomizing Donkey",	I(donkey),					I(lobodonkey),	P_.LOBOTOMY,						-1,			-1);
