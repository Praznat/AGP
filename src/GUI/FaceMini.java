...
class FaceMini extends JPanel {
	...
	public void loadAttributes(Ideology I) {
		NOSERX = fix(I.getBeh(M_.NOSERX), 3, 20);
		NOSERY = fix(I.getBeh(M_.NOSERY), -5, 10);
		NOSELX = fix(I.getBeh(M_.NOSELX), 3, 20);
		NOSELY = fix(I.getBeh(M_.NOSELY), -5, 20);
		NOSELW = fix(I.getBeh(M_.NOSELW), 5, 25);
		NOSEMW = fix(I.getBeh(M_.NOSEMW), -7, 8);
		NOSERW = fix(I.getBeh(M_.NOSERW), -7, 8);
		MOUTHBX = fix(I.getBeh(M_.MOUTHBX), 0, 20);
		MOUTHBY = fix(I.getBeh(M_.MOUTHBY), 4, 20);
		MOUTHP = fix(I.getBeh(M_.MOUTHP), 0, 15);
		MOUTHC = fix(I.getBeh(M_.MOUTHC), -50, 120);
		MOUTHLH = fix(I.getBeh(M_.MOUTHLH), 2, 10);
		MOUTHLW = fix(I.getBeh(M_.MOUTHLW), 10, 60);
		MOUTHJH = fix(I.getBeh(M_.MOUTHJH), 0, 10);
		MOUTHJW = fix(I.getBeh(M_.MOUTHJW), 10, 40);
		EYERP = fix(I.getBeh(M_.EYERP), 0, 50);
		EYELP = fix(I.getBeh(M_.EYELP), 0, 50);
		EYERW = fix(I.getBeh(M_.EYERW), 10, 30);
		EYELW = fix(I.getBeh(M_.EYELW), 10, 30);
		EYEHGT = fix(I.getBeh(M_.EYEHGT), 0, 10);
		EYESPRD = fix(I.getBeh(M_.EYESPRD), 0, 10);
		EARH = fix(I.getBeh(M_.EARH), 0, 50);
		EARW = fix(I.getBeh(M_.EARW), 10, 50);
		EART = fix(I.getBeh(M_.EART), 0, 15);
		EARD = fix(I.getBeh(M_.EARD), 20, 80);
		SKINR = fix(I.getBeh(M_.SKINR), 0, 99);
		SKING = fix(I.getBeh(M_.SKING), 50, 239);
		SKINB = fix(I.getBeh(M_.SKINB), 50, 239);
		HAIRL = fix(I.getBeh(M_.HAIRL), 0, 30);
		HAIRC = fix(I.getBeh(M_.HAIRC), 0, 89);
		HAIRW = fix(I.getBeh(M_.HAIRW), 0, 10);
		HAIRR = fix(I.getBeh(M_.HAIRR), 0, 255);
		HAIRG = fix(I.getBeh(M_.HAIRG), 0, 255);
		HAIRB = fix(I.getBeh(M_.HAIRB), 0, 255);
		HAIRX = fix(I.getBeh(M_.HAIRX), 20, 99);
		HAIRS = fix(I.getBeh(M_.HAIRS), 0, 9);
	}
